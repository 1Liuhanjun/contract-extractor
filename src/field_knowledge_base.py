...
按业务人员确认的规则重写。改动要点：
  ...
  - 合同类型：乙方第一中标人→主选, 乙方第二/第三中标人→备选, 默认→主选
  - 合同名称：第一条线路名 + "一干运输合同"
...
FIELD_KNOWLEDGE_BASE = {
    ...
    "合同类型": {
        "description": "主选/备选（按乙方中标/成交身份判断）", "source": "contract → mapping",
        ...
        "mapping_rules": [
            ...
            {"condition": "维度1—中标人(乙方是)：乙方公司名附近的描述含'第一中标人/第一成交人(主供应商)'→有主选身份",
             "rule": "主选侧 = 是"},
            {"condition": "维度1—中标人(乙方是)：乙方被列为'第二中标人/第二成交人/第三中标人/第三成交人'→有备选身份",
             "rule": "备选侧 = 是"},
            {"condition": "最终判定：乙方同时有第一和第二/第三中标/成交身份 → '主选/备选'",
             "rule": "双阳性才写主选/备选，线路备用/临时邮路不参与合同类型判断"},
            {"condition": "只有主选侧=是 → '主选'",
             "rule": "→ '主选'"},
            {"condition": "只有备选侧=是 → '备选'",
             "rule": "→ '备选'"},
        ],
        "note": "⚠️ 验证规则：甲方一定是客户侧（邮政/京东等），合同类型只判断乙方公司是第几中标/成交人。提取到的中标人描述必须绑定乙方公司全称或明确写'乙方作为第X中标人'，否则丢弃。例如：'江苏省分公司...第一成交人'是甲方采购描述，不代表乙方主选。乙方第一=主选；乙方第二/第三=备选；主选/备选必须同时有乙方第一和乙方第二/第三两侧有效证据；线路备用、临时邮路、标包号不参与合同类型判断。若最终无有效证据且模型输出空值，则按高频默认值兜底为主选。",
    },
    "合同开始时间": {
